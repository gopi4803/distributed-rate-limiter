...
public record RateLimitRule(String name, int limit, Duration window,Algorithm algorithm) {
    public RateLimitRule{
        ...
        if(name.isBlank()) throw new IllegalArgumentException("Rate limit rule name must not be blank");
        if(limit<=0) throw new IllegalArgumentException("Rate limit rule limit must be greater than zero");
        if(window.isZero() || window.isNegative()) throw new IllegalArgumentException("Rate limit rule window must be greater than zero");
